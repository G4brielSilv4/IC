...
%% TIRANTE X %Contraventamento do plano da cobertura - Em formato de X
[ppcontvert, preco_tirante_x_sec,pen_contraventamento,peso_tirante_x_sec]= tirante_x (perfil_tirante_x,fu,fy,incl,Cpi2,p,espacamento_entre_porticos,altura_cobertura,esp_terca,h,n_tercas); %N/m

tcv =zeros(n_nos,3);% [nó   intensidade_x  intensidade_y] inicialmente zeros
for n=1:n_nos
    tcv(n,1)= n;
end
tcv((n_divisoes_banzo_inf+1)*2+2,3)= -ppcontvert*(espacamento_entre_porticos^2+esp_terca^2)/2;
tcv(n_nos,3)= tl((n_divisoes_banzo_inf+1)*2+2,3);
for n=(n_divisoes_banzo_inf+1)*2+2:terca_a_cada_divisao_banzo_sup:(n_divisoes_banzo_inf+1)*2+2*n_divisoes_banzo_sup
    if n~=(n_divisoes_banzo_inf+1)*2+2
        tcv(n,3)= -ppcontvert*(espacamento_entre_porticos^2+esp_terca^2);
    end
end
...
penalidade = sum(pen)+ sum(pen_desloc)+ pen_telha+pen_contraventamento+pen_tirante+pen_terca+pen_mao_francesa+pen_trelica_s+pen_trelica_d;
...
peso_tirante_x_total=peso_tirante_x_sec*2;
...
preco_tirante_x_total=preco_tirante_x_sec*2;
...
% xvar(1-4) --> perfil do elemento da treliça              -> FICA
% xvar(5)   --> perfil da telha                            -/-/-/-/-/-RETIRAR-/-/-/-/-/-/-              
% xvar(6)   --> perfil da terça                            -/-/-/-/-/-RETIRAR-/-/-/-/-/-/-
% xvar(7)   --> diâmetro do tirante Y                      -/-/-/-/-/-RETIRAR-/-/-/-/-/-/-
% xvar(8)   --> diâmetro do tirante X                      -/-/-/-/-/-RETIRAR-/-/-/-/-/-/-
% xvar(9)   --> espaçamento entre banzos da treliça        -> FICA
% xvar(10)  --> altura da cobertura                        -> FICA
% xvar(11)  --> espaçamento entre porticos                 -/-/-/-/-/-RETIRAR-/-/-/-/-/-/-
% xvar(12)  --> espaçamento entre nós (em função da telha) -> FICA
% xvar(13)  --> número de divisões do banzo inferior       -> FICA
% xvar(14)  --> número de terças                           -/-/-/-/-/-RETIRAR-/-/-/-/-/-/-
% xvar(15)  --> número de contraventamentos                -/-/-/-/-/-RETIRAR-/-/-/-/-/-/-
% xvar(16)  --> perfil da cantoneira da mão francesa       -/-/-/-/-/-RETIRAR-/-/-/-/-/-/-
global preco_final

if custo_total<preco_final
    tipo_telha=round(xvar(5));   %xvar 5
    tipo_terca=round(xvar(6));   %xvar 6
    dia_Y=round(xvar(7));        %xvar 7
    dia_X=round(xvar(8));        %xvar 8
    esp_ent_port=round(xvar(11));%xvar 11
    num_tercas=round(xvar(14));  %xvar 14
    num_cont=round(xvar(15));    %xvar 15
    perfil_cant=round(xvar(16)); %xvar 16
    
    assignin('base','tipo_telha',tipo_telha)
    assignin('base','tipo_terca',tipo_terca)
    assignin('base','dia_Y',dia_Y)
    assignin('base','dia_X',dia_X)
    assignin('base','esp_ent_port',esp_ent_port)
    assignin('base','num_tercas',num_tercas)
    assignin('base','num_cont',num_cont) 
    assignin('base','perfil_cant',perfil_cant)
